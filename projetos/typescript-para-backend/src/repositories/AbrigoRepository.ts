...
export default class AbrigoRepository implements InterfaceAbrigoRepository {
  constructor(private readonly repository: Repository<AbrigoEntity>) {}
  ...
  async atualizaAbrigo(id: number, newData: AbrigoEntity): Promise<void> {
  ...
  async deletaAbrigo(id: number): Promise<void> {
